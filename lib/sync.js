...
class SyncManager {
  ...
  _buildDocument(post) {
    ...
    if (this.options.fields.includes('content')) {
      // Strip HTML for indexing and limit to 5000 chars to avoid metadata size limits
      let content = post.content?.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() || '';
    ...
    if (this.options.fields.includes('excerpt')) {
      let excerpt = post.excerpt?.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() || '';
